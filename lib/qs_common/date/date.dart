...
class DateUtil {
  ...
  static String getNowDateYMDStr() {
    return getDateStrByDateTime(DateTime.now(),
  ...
  static String readTimestamp(int timestamp) {
    ...
    // print(
    //     'timestamp-now.year-data.year:$timestamp -- ${now.year} -- ${date.year} -- ${getNowDateMs()}');
    // 同一年
      ...
      if (diff.inDays == 0) {
        if (diff.inHours == 0) {
          ...
          if (diff.inMinutes == 0) {
            ...
            time = diff.inMinutes < 0 ? '刚刚' : '${diff.inMinutes}分钟前';
          }
        } else {
